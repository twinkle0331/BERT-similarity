...
def main():
    ...
    def compute_similarity(features_x, features_y, methods, weight=None, fit_model=None):
        if methods == "SVCCA":
            cca = CCA()
    ...
    output_dir = os.path.join(training_args.output_dir, f"sanity_check/methods_{methods}")
    os.makedirs(output_dir, exist_ok=True)

    if model_args.train:
        fixed_fit_model = INN.Sequential(INN.Nonlinear(768, 'RealNVP')).to(training_args.device)
        fixed_fit_model.apply(weights_init)
        if methods =="INN":
    ...
    if model_args.train:
        if methods == "INN" or methods == "Linear":
            for iter in tqdm(range(3000)):
                try:
                    ...
                    for i in range(num_layer):
                        ...
                        selected_y,_,_ = fixed_fit_model(selected_x)
                        if methods == "INN":
                            y_pre, logp, logdet = fit_model(selected_y)
                        else:
                            y_pre = fit_model(selected_y)
                        MSELoss = torch.nn.MSELoss()
                        loss = MSELoss(y_pre, selected_x)
