...
def main():
    ...
    def extract_hidden_states_to_train_fit_model(model, inputs, batch_size):
        """
    ...
    def compute_similarity(features_x,features_y,methods,fit_model=None,weight=None):
        if methods == "SVCCA":
            ...
            y_pre = fit_model(features_x)
            mse_loss = torch.nn.MSELoss()
            score = mse_loss(y_pre, features_y)
        elif methods == "NonBijINN":
            y_pre = fit_model(features_x)
            mse_loss = torch.nn.MSELoss()
            score = mse_loss(y_pre, features_y)
        return score

    def compute_hidden_similarity(tensor_x, tensor_y, methods, weight=None, fit_model=None):
        num_layer = tensor_x.shape[0]
        if weight==None and (methods == "PWCCA" or methods == "SVCCA"):
            similarity = [[[] for col in range(num_layer)] for row in range(num_layer)]
        else:
            similarity = np.zeros((num_layer, num_layer))
        for i in tqdm(range(num_layer)):
            for j in range(i + 1):
                selected_x = tensor_x.select(0, i)
                selected_y = tensor_y.select(0, j)
                if type(weight) == np.ndarray:
                    weight_item = weight[i][j]
                else:
                    weight_item = None
                similarity[i][j] = compute_similarity(selected_x, selected_y, methods, fit_model=fit_model, weight=weight_item)
                # logger.info(f"Similarity of layer {i} and layer {j} is {similarity[i][j]}")
    ...
    if model_args.train:
        if methods =="INN":
            ...
            fit_model = nn.Sequential(nn.Linear(768, 768)).to(training_args.device)
        elif methods == "NonBijINN":
            fit_model = MLP().to(training_args.device)
    ...
    if model_args.train:
        if methods == "INN" or methods == "Linear" or methods == "NonBijINN":
            for iter in tqdm(range(3000)):
                try:
                    step, inputs = train_dataloader_iter.__next__()
                    optimizer.zero_grad()
                    hidden_states_x = extract_hidden_states_to_train_fit_model(model, inputs, batch_size=training_args.per_device_train_batch_size)
                    hidden_states_y = extract_hidden_states_to_train_fit_model(model_y, inputs, batch_size=training_args.per_device_train_batch_size)
            ...
            hidden_states_x = extract_hidden_states(model=model, train_dataloader=train_dataloader,
                                                    step_num=step_num, batch_size=training_args.per_device_train_batch_size)
            hidden_states_y = extract_hidden_states(model=model_y, train_dataloader=train_dataloader,
                                                    step_num=step_num, batch_size=training_args.per_device_train_batch_size)
            fit_model = None
            weight = compute_hidden_similarity(hidden_states_x, hidden_states_y, methods=methods, fit_model=fit_model)
        ...
        with torch.no_grad():
            ...
            hidden_states_x = extract_hidden_states(model, train_dataloader, step_num, batch_size=training_args.per_device_train_batch_size)
            hidden_states_y = extract_hidden_states(model_y, train_dataloader, step_num, batch_size=training_args.per_device_train_batch_size)
            similarity = compute_hidden_similarity(hidden_states_x, hidden_states_y, methods, fit_model, weight)
