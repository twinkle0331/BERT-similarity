...
class CCA():
    ...
    def compute_ccas(self, sigma_xx, sigma_xy, sigma_yx, sigma_yy, epsilon,
        ...
        weight_x = np.dot(invsqrt_xx, u)
        weight_y = np.dot(invsqrt_yy, v)


        if verbose:
            print("computed everything!")

        return [u, np.abs(s), v], invsqrt_xx, invsqrt_yy, x_idxs, y_idxs, weight_x, weight_y
    ...
    def get_cca_similarity(self, acts1, acts2, weight, epsilon=1e-20, threshold=0.98,
                        compute_coefs=True,
                        compute_dirns=False,
                        verbose=True):
        ...
        x_idxs, y_idxs, weight_x, weight_y) = self.compute_ccas(sigmaxx, sigmaxy, sigmayx, sigmayy,
                                        epsilon=epsilon,
                                        verbose=verbose)
        ...
        if not type(weight) == np.ndarray:
            ...
            cca_mse_loss = np.mean((acts2.T - np.dot(np.dot(acts1.T, weight[0]), np.linalg.pinv(weight[1]))) ** 2)

        # if x_idxs or y_idxs is all false, return_dict has zero entries
        if (not np.any(x_idxs)) or (not np.any(y_idxs)): # skip
        ...
        return_dict['cca_mse_loss'] = cca_mse_loss
    ...
    def compute_svcca(self, X, Y, weight):
        '''
        Args:
            X neuron * num_sample
            Y neuron * num_sample
        Return:
            cca simialrity score
        '''
        X,Y =X.T,Y.T
        ...
        U1,s1,V1 = svd(X,full_matrices=False)
        U2,s2,V2 = svd(Y,full_matrices=False)
        idx1 = self.sum_threshold(s1,0.9)
        idx2 = self.sum_threshold(s2,0.9)
        idx = max(idx1,idx2)

        svx = np.dot(s1[:idx]*np.eye(idx),V1[:idx])
        svy = np.dot(s2[:idx]*np.eye(idx),V2[:idx])

        svcca_results = self.get_cca_similarity(svx,svy,weight,epsilon=1e-20,verbose=False)
        # print(f"Fraction of variance explained by {idx} singular vectors", np.sum(s1[:idx]) / np.sum(s1))
        svcca_score = np.mean(svcca_results["cca_coef1"])
        if type(weight) == np.ndarray:
            svcca_mse_loss = svcca_results["cca_mse_loss"]
            return svcca_mse_loss
        else:
            return svcca_results["weight"]


    def compute_pwcca(self, acts1, acts2, weight, epsilon=1e-10):
        ...
        """
        acts1, acts2 = acts1.T, acts2.T

        acts1 = acts1 - np.mean(acts1, axis=1, keepdims=True)
        acts2 = acts2 - np.mean(acts2, axis=1, keepdims=True)
        sresults = self.get_cca_similarity(acts1, acts2, weight, epsilon=epsilon,
        ...
        direct_weight = np.abs(np.dot(P.T,acts[idxs].T))
        weights = weights / np.sum(weights)
        pwcca_score = np.sum(weights * coefs)
        if type(weight) == np.ndarray:
            # pwcca_mse_loss = sresults["cca_mse_loss"]
            return sresults['inference']
        else:
            return sresults["weight"]
...
class CKA():
    ...
    def gram_rbf(self, x, threshold=1.0):
    ...
    def cca(self, features_x, features_y):
    ...
    def linear_cka(self, features_x, features_y,debiased=False):
        num_example, num_feature = min(features_x.shape,features_y.shape)
        if num_example<num_feature:
            linear_cka_score = self.cka(self.gram_linear(features_x), self.gram_linear(features_y),debiased=debiased)
        else:
            linear_cka_score = self.cka(self.gram_linear(features_x), self.gram_linear(features_y),debiased=debiased)
        return linear_cka_score
    
    def rbf_cka(self, features_x,features_y):
        num_example, num_feature = min(features_x.shape,features_y.shape)
        if num_example<num_feature:
            rbf_cka_score = self.cka(self.gram_rbf(features_x), self.gram_rbf(features_y),debiased=True)
        else:
            rbf_cka_score = self.cka(self.gram_rbf(features_x), self.gram_rbf(features_y),debiased=False)
        return rbf_cka_score
